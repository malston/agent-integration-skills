...
(diagram omitted in bundle: Pipeline — Input flows through Agent A (Plan), Agent B (Execute), Agent C (Verify) sequentially to produce Output)
...
Runnable implementation: [samples/python/routing/pipeline.py](sample code not bundled)
...
- *is-a* [Orchestrator](orchestrator.md) — a linear orchestrator is a pipeline.
- *alternative-to* [Scatter-Gather](scatter-gather.md) — sequential vs. parallel.
- *used-by* [Checkpoint & Resume](checkpoint-resume.md) — pipeline steps are natural checkpoint boundaries.
