...
(diagram omitted in bundle: Agent Proxy — Client Agent sends request to Agent Proxy, which authenticates, transforms, and forwards to the remote Agent B)
...
Runnable implementation: [samples/python/discovery/agent_proxy.py](sample code not bundled)
...
- *complements* [Trust Boundary](trust-boundary.md) — the proxy is often the gateway tier implementation.
- *complements* [Circuit Breaker](circuit-breaker.md) — proxy + circuit breaker = a robust remote agent façade.
