...
(diagram omitted in bundle: Blackboard — shared context space with Agent A, B, and C all reading and writing)
...
## Sample Code
Runnable implementation: [samples/python/messaging/blackboard.py](sample code not bundled)
...
## When to avoid
- When agents need to react to each other in real time — use [Group Chat](group-chat.md) instead.
- When strong write ordering is required — use a Pipeline.
- When the problem is strictly sequential — use [Pipeline](pipeline.md).
...
## Related Patterns
- *alternative-to* [Group Chat](group-chat.md) — shared mutable state rather than a conversation thread.
- *complements* [Checkpoint & Resume](checkpoint-resume.md) — the blackboard is a natural checkpoint target.
