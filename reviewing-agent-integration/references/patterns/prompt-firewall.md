...
(diagram omitted in bundle: Prompt Firewall — External Data (untrusted) passes through the Prompt Firewall (sanitize/flag) before reaching Agent Context (trusted input))
...
Runnable implementation: [samples/python/security/prompt_firewall.py](sample code not bundled)
...
- *used-by* [Agent Proxy](agent-proxy.md) — the proxy is the natural location for a firewall on incoming requests.
