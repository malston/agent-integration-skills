...
(diagram omitted in bundle: Tool Provider — Agent calls tools/list to discover available tools, then tools/call to invoke them; MCP Tool Server returns structured results)
...
Runnable implementation: [samples/python/context/tool_provider.py](sample code not bundled)
...
- *complements* [Least-Privilege Tool Scope](least-privilege-tool-scope.md) — Tool Provider defines the available tools; Least-Privilege scopes which agent gets which subset.
...
- *used-by* [Magentic Orchestration](magentic.md) — specialists in Magentic are typically tool-equipped agents.
