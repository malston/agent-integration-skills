...
- **F11 Operational complexity** — every step requires a compensation, doubling the code. Compensations must be idempotent (see [Idempotent Agent](idempotent-agent.md)) and may not perfectly undo the original action (e.g., a sent email cannot be unsent — use a notification instead).
...
## Sample Code
Runnable implementation: [samples/python/coordination/saga.py](sample code not bundled)
...
## Related Patterns
- *uses* [Idempotent Agent](idempotent-agent.md) — compensation steps must be idempotent; repeated compensation must not double-undo.
- *uses* [Checkpoint & Resume](checkpoint-resume.md) — the saga step log is the natural checkpoint; resume can re-enter the compensation path.
- *used-by* [Dead Letter Agent](dead-letter-agent.md) — if compensation itself fails, the failed compensation routes to the dead letter handler.
- *alternative-to* [Pipeline](pipeline.md) — a pipeline with no rollback vs. a saga with explicit compensation.
