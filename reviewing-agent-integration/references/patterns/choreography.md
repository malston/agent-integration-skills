...
(diagram omitted in bundle: Choreography — task_created triggers Agent A, which publishes data_extracted; Agent B reacts and publishes analyzed; Agent C finalizes to done — no central coordinator)
...
Runnable implementation: [samples/python/coordination/choreography.py](sample code not bundled)
...
- *used-by* [Dead Letter Agent](dead-letter-agent.md) — unhandled events route to the dead letter handler.
- *complements* [Blackboard](blackboard.md) — events trigger agents; blackboard stores accumulated state.
