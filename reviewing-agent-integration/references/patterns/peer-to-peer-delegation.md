...
(diagram omitted in bundle: Peer-to-Peer Delegation — Agent A discovers Agent B via Agent Registry, sends A2A Task Request, receives A2A Task Result without a central coordinator)
...
Runnable implementation: [samples/python/coordination/peer_to_peer_delegation.py](sample code not bundled)
...
- *uses* [Agent Card Registry](agent-card-registry.md) — to discover capable peers.
- *alternative-to* [Supervised Delegation](supervised-delegation.md) — no supervisor; agents self-organize.
- *complements* [Trust Boundary](trust-boundary.md) — trust tiers are essential when peer delegation crosses security zones.
