...
(diagram omitted in bundle: Magentic)

## Sample Code

Runnable, tested implementation: [`samples/coordination/magentic/`](sample code not bundled)
...
- *uses* **[Checkpoint & Resume](checkpoint-resume.md)** — the task ledger is a natural checkpoint boundary for long, expensive runs.
- *uses* **[Dead Letter Agent](dead-letter-agent.md)** — escalation target when the stall limit is reached.
