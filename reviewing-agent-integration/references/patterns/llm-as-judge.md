...
Runnable implementation: [samples/python/evaluation/llm_as_judge.py](sample code not bundled)
...
- *used-by* [Pipeline](pipeline.md) — the verifier step in a pipeline is typically an LLM-as-Judge.
- *used-by* [Magentic Orchestration](magentic.md) — the manager can use a judge to verify specialist outputs.
- *complements* [Dead Letter Agent](dead-letter-agent.md) — rejected outputs after max retries route to dead letter.
- [Scatter-Gather](scatter-gather.md) — Ensemble Judge is a specialization of Scatter-Gather for evaluation
