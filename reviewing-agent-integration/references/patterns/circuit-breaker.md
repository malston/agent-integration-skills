...
(diagram omitted in bundle: Circuit Breaker — Agent A call passes through Circuit Breaker with three states: CLOSED passes to Agent B (healthy), OPEN returns Error (fast), HALF probe tests Agent B (testing))
...
Runnable implementation: [samples/python/resilience/circuit_breaker.py](sample code not bundled)
...
- *complements* [Agent Proxy](agent-proxy.md) — the proxy is where the circuit breaker is typically implemented.
- *used-by* [Magentic Orchestration](magentic.md) — stall detection in Magentic is a soft circuit breaker for emergent plans.
