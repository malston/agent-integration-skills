...
(diagram omitted in bundle: Content-Based Router — Incoming Task flows into Router Agent (LLM classifier or rule engine), which routes to Coding Agent, Research Agent, or Data Agent)
...
Runnable implementation: [samples/python/routing/content_based_router.py](sample code not bundled)
...
- *uses* [Agent Card Registry](agent-card-registry.md) — registry maps capability to agent address.
- *used-by* [Peer-to-Peer Delegation](peer-to-peer-delegation.md) — P2P delegation uses content-based routing to find the right peer.
