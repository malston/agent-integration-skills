...
(diagram omitted in bundle: Direct Message — Agent A sends Task Request to Agent B, receives Task Result)

## Sample Code
Runnable implementation: [samples/python/messaging/direct_message.py](sample code not bundled)
...
## When to avoid
- When the receiver identity may change at runtime — use [Agent Card Registry](agent-card-registry.md) + [Content-Based Router](content-based-router.md) instead.
...
## Related Patterns
- *used-by* [Supervised Delegation](supervised-delegation.md) — the supervisor delegates via Direct Message.
- *alternative-to* [Broadcast Message](broadcast-message.md) — when exactly one agent should handle the task.
- *uses* [Agent Card Registry](agent-card-registry.md) — to discover the receiver address before sending.
