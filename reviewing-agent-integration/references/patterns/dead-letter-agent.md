...
(diagram omitted in bundle: Dead Letter Agent — Agent A failure is routed to the Dead Letter Agent, which triggers Human review / alert)
...
Runnable implementation: [samples/python/resilience/dead_letter_agent.py](sample code not bundled)
...
- *used-by* [Ensemble Judge](ensemble-judge.md) — rejected outputs after max retries route to dead letter.
- *used-by* [Group Chat](group-chat.md) — tasks that exceed the iteration cap escalate here.
- *used-by* [Magentic Orchestration](magentic.md) — stall escalation target.
