...
- [Dead Letter Agent](dead-letter-agent.md) — escalation target when the loop exhausts its iteration budget.
- [Checkpoint & Resume](checkpoint-resume.md) — persist the best draft across iterations so a crash does not restart from zero.
- [Circuit Breaker](circuit-breaker.md) — open the breaker if the Critic is systematically unavailable to avoid burning the iteration budget.
