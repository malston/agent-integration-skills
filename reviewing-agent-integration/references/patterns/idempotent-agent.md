...
## Sample Code
Runnable implementation: [samples/python/implementation/idempotent_agent.py](sample code not bundled)
...
- *used-by* [Saga / Compensating Action](saga.md) — saga steps must be idempotent so that compensation can safely run after a partial failure.
