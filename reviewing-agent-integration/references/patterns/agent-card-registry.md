...
(diagram omitted in bundle: Agent Card Registry — Agent A queries the registry with "who can extract PDFs?", receives Agent B's endpoint, then connects directly)
...
Runnable implementation: [samples/python/discovery/agent_card_registry.py](sample code not bundled)
...
- *used-by* [Peer-to-Peer Delegation](peer-to-peer-delegation.md) — agents use the registry to find capable peers.
- *used-by* [Content-Based Router](content-based-router.md) — the router resolves capability to agent address via the registry.
