...
(diagram omitted in bundle: Least-Privilege Tool Scope — Research Agent connects via MCP to read-only server (search_web, read_document); Execution Agent connects via MCP to write-scope server (write_file, run_code))
...
Runnable implementation: [samples/python/security/least_privilege_tool_scope.py](sample code not bundled)
...
- *used-by* [Tool Provider](tool-provider.md) — the tool provider implements the allowlist enforcement.
