...
(diagram omitted in bundle: Group Chat)

## Sample Code

Runnable, tested implementation: [`samples/coordination/group_chat/`](sample code not bundled)
...
- *alternative-to* **[Scatter-Gather](scatter-gather.md)** — parallel, no cross-talk; choose it when independent perspectives suffice.
- *alternative-to* **[Blackboard](blackboard.md)** — shared mutable *state* rather than a *conversation*; Group Chat is dialogue, Blackboard is a data store.
- *refines-into* **[Ensemble Judge](ensemble-judge.md)** — when "discussion" reduces to independent verdicts, prefer the cheaper parallel ensemble.
- *uses* **[Dead Letter Agent](dead-letter-agent.md)** — escalation target when the iteration cap is hit without approval.
