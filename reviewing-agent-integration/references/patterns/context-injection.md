...
(diagram omitted in bundle: Context Injection — Host assembles context from MCP Resources via Context Assembler, then injects it into the Agent)
...
Runnable implementation: [samples/python/context/context_injection.py](sample code not bundled)
...
- *complements* [Blackboard](blackboard.md) — blackboard stores mutable shared state; context injection fetches read-only resources.
- *used-by* [Orchestrator](orchestrator.md) — each step receives state injected from the orchestrator.
